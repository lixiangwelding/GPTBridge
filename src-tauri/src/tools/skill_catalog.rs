...
#[derive(Clone,Debug)]
pub struct Catalog {workspace:PathBuf, globals:Vec<PathBuf>, pub enabled:bool}
#[derive(Clone)]
...
pub struct Scan {pub skills:Vec<Skill>,pub truncated:bool,pub skipped:usize,pub visited:usize,pub root_count:usize}
impl Skill {
...
impl Catalog {
    pub fn new(workspace:PathBuf,globals:Vec<PathBuf>)->Self{Self{workspace,globals,enabled:true}}
    ...
    pub fn scan(&self)->Result<Scan,WorkspaceError>{
        ...
        let mut scan=Scan{skills:Vec::new(),truncated:false,skipped:0,visited:0,root_count:roots.len()};
            ...
            if raw.exists(){
                if let Ok(file)=raw.canonicalize(){
                    if file.starts_with(&real) && seen_files.insert(file.clone()){
                        ...
                        read_bytes+=size;
                        match io::read(&file).and_then(|text|io::metadata(&text).map(|meta|(text,meta))){
                            Ok((text,meta))=>{
                                ...
                                scan.skills.push(Skill{id:digest(format!("{}\0{}",self.workspace.display(),file.display()))[..32].into(),
                                    name:meta.name,description:meta.description,alias:real.file_name().unwrap_or_default().to_string_lossy().to_string(),scope,root,file,sha:digest(text.as_bytes())});
    ...
    pub fn list(&self,args:&Value)->Result<Value,WorkspaceError>{
        ...
        Ok(json!({"ok":true,"skills":page,"matches":ranked.len(),"next_cursor":if next<ranked.len(){Some(format!("{revision}:{next}"))}else{None},
            ...
            "hot_reload":"rescan_each_call","native_dollar_picker":false,"scripts_executed":false}))
    }
