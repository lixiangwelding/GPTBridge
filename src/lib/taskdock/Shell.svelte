<script lang="ts">
  ...
  const entries = [{ path: "/", title: "工作台", icon: LayoutDashboard }, { path: "/projects", title: "项目", icon: Folder }, { path: "/skills", title: "工具与技能", icon: Sparkles }, { path: "/settings", title: "设置", icon: Settings2 }];
  const active = (path: string) => path === "/" ? $page.url.pathname === "/" : $page.url.pathname.startsWith(path) || (path === "/settings" && $page.url.pathname.startsWith("/workspace/"));
