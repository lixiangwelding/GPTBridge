...
<svelte:head><title>设置 · TaskDock</title></svelte:head>
...
<div class="td-setting-grid"><section class="td-panel">
  <div class="td-setting-row"><div><h2>连接你的 AI 工具</h2><p>推荐 MCP 原生连接。客户端握手、本地监听和本实例状态分别说明。</p></div><button class="td-button small" onclick={() => connectionProject.set($workspaces[0]?.id || "")}>管理连接</button></div>
  ...
  <details class="td-details"><summary>兼容说明：旧连接和任务数据</summary><p>TaskDock 是新的产品名。稳定的应用标识、协议工具名、配置目录、命令行入口和任务数据库保留兼容，避免改名后丢配置、重复创建数据库或中断旧连接。不会因为打开新版页面而替换正在运行的服务。</p></details>
</section><aside class="td-panel td-help"><div class="td-eyebrow">SET ONCE. GET BACK TO WORK.</div><h2 style="margin-top:12px">少操作，不少边界。</h2><p>任务入口负责目标与进展，AI 客户端负责继续执行。创建目标不是生成代码，命令完成也不是业务验收通过。</p><p>配置收起来，能力留下来。只有项目真的不一样时，才进入高级配置修改。</p><span class="td-pill">TaskDock · AI 任务工作台</span></aside></div>
