...
**连接 ChatGPT 与本地项目、工具和技能的 AI 任务工作台。** 将任务进展、接续上下文和执行记录集中到一处，少一点重复配置，多一点持续推进。

[中文](README.md) · [English](README.en.md) · [使用与兼容说明](PERSONAL.md) · [品牌与版本边界](docs/gptbridge/BRANDING.md)

## 五个入口，一处掌握

| 入口 | 能做什么 |
| --- | --- |
| 工作台 | 查看真实任务记录，搜索、筛选和分页；在详情中读取检查点、作业状态与日志 |
| 项目 | 选择本地目录，切换项目上下文，集中管理连接和高级配置 |
| MCP 连接 | 直接进入所选项目的连接配置，查看端口、认证、隧道和明确授权的 Skill 写入目录 |
| 工具与技能 | 搜索本地技能、查看来源与工具合同，设置项目级自动匹配偏好 |
| 设置 | 调整常用选项；端口、认证、隧道和兼容配置按需展开 |

## 开始与接续

![GPTBridge 0.4.1 工作台：实际前端构建的浏览器预览](docs/gptbridge/screenshots/browser-preview-0.4.1/workbench-desktop.png)

**历史截图环境：0.4.1 实际前端构建的浏览器预览，早于新增“MCP 连接”一级入口；Tauri 原生服务未连接，没有注入演示任务。** 页面保留当时的未连接状态，不是本次最终界面、原生后端或任务执行验收。查看 [四个页面的历史桌面与窄屏截图](docs/gptbridge/SCREENSHOTS.md) 及 [验证记录](docs/gptbridge/VALIDATION.md)。

选择项目 → 配置客户端连接 → 描述任务目标 → 查看进展 → 在已连接的 AI 客户端继续。
...
这是源码更新，不代表已完成原生桌面验收、安装替换或签名安装包发布。最新截图须按其证据说明区分原生应用、前端浏览器预览和历史设计图；不能将它们互相替代。
...
仅运行前端预览不会连接 Tauri 原生服务；任务数据不会因此自动出现。旧设计规格保留在 [0.4.0 工作台规格](docs/specs/taskdocksvelte5tauri2/requirements.md)，历史名称不作追溯性修改。

## 来源与独立性

本项目是 `lixiangwelding` 维护的独立项目，基于 `mybolide/coding-tools-mcp` 的 Rust/Tauri 实现演进，保留上游许可证及来源说明。GPTBridge 不是 OpenAI 官方产品，也不代表官方认可或背书。ChatGPT 兼容性与品牌归属是不同事项。

<details>
<summary>历史参考：个人版与上游旧界面、安装和接入资料</summary>

以下内容作为历史参考保留。旧截图、上游安装包、菜单位置与版本结论不代表 GPTBridge 0.4.1 的当前验收状态；日常入口以上文及当前源码为准。

## 个人版历史说明

0.3.3 修复 `/mcp` 事件流 GET 的协议响应；以 `Accept: text/event-stream` 请求时返回 `405`，普通健康检查仍可读取版本 JSON。

0.3.2 新增 [本机 Skill 搜索与引用](docs/local-skills.md)：直接读取现有技能目录，支持发送 `$技能名` 后加载，无需复制原件或额外服务。这里的 `$` 是发送后的文本别名，不是 ChatGPT 原生输入框下拉。

这是 `lixiangwelding` 的独立个人二开版。**先看 [个人版使用与验收说明](PERSONAL.md)**：无需复制初始化长提示词、同目录多任务不使用 worktree、持久任务与命令恢复、独立配置与端口。

0.3.1 新增 [一条 FRP／一个 MCP 入口共享多个仓库](docs/shared-gateway.md)，并修复配置兼容、任务创建原子性和同文件多段补丁丢失改动等审查问题。默认不启用共享，不更改正在运行的旧服务。
...
除了本应用内置的开发工具，工作区还可以受控地代理管理员明确配置的 MCP。支持本机进程的 **stdio**，以及本机或可达服务的 **Streamable HTTP**。ChatGPT 或云端 Codex 始终只连接本应用原有的公网 `/mcp` 地址，桌面端再把调用转发给已保存的上游。

在工作区的 **MCP → 配置 → 本地 MCP** 中点击 **“+ 添加”**：

1. 选择 **快速创建**，再选择 `stdio` 或 `Streamable HTTP` 传输。
2. 或选择 **从 JSON 导入**，粘贴单个配置、配置数组，或常见的 `mcpServers` 对象；导入只生成草稿，须审阅后保存。
3. `stdio` 填写命令、参数和环境变量；`Streamable HTTP` 填写端点 URL 和可选请求头。
4. 勾选该本地 MCP 的“启用”开关，并点击“获取工具”。
5. 默认所有已发现工具都会公开；按需要逐项关闭高权限工具。
6. 保存。若 MCP 已在运行，桌面端会重新加载；否则在下次启动 MCP 时生效。

Streamable HTTP 导入示例：
...
远端客户端看到的工具会带工作区配置的前缀，例如 `local__tool_name`。旧版手工白名单会保持原有公开范围，直到在新版界面保存逐项开关。

> 安全边界：环境变量仅注入本机 stdio 进程；HTTP 请求头仅发送到其保存的端点。两者都不会出现在 `/mcp` 工具目录、工具响应或普通请求日志中。自动公开会让远端获得该服务提供的所有本机能力；请在保存前逐项关闭不希望暴露的工具，且不要配置不受信任的 MCP 或包含敏感凭据的环境变量。
