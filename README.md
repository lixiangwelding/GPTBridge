...
**截图环境：0.4.1 实际前端构建的浏览器预览，Tauri 原生服务未连接，没有注入演示任务。** 页面保留真实的未连接状态；这不是原生后端或任务执行验收。查看 [四个页面的桌面与窄屏截图](docs/gptbridge/SCREENSHOTS.md) 及 [验证记录](docs/gptbridge/VALIDATION.md)。
